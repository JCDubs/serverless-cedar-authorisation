...
export class UserDetailService {
  ...
  static setUserDetails(authenticatedEvent: APIGatewayProxyEvent) {
    ...
    this.userName = authenticatedEvent.requestContext.authorizer.claims['cognito:username'];
    ...
    if (typeof authenticatedEvent.requestContext.authorizer.claims['cognito:groups'] === 'string') {
      this.roles = [];
      this.roles.push(authenticatedEvent.requestContext.authorizer.claims['cognito:groups']);
    } else {
      this.roles = authenticatedEvent.requestContext.authorizer.claims['cognito:groups'];
    }
